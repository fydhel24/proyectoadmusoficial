...
class CamarografoController extends Controller
{
    /*  public function tareasSemanaActual()
    {
        ...
        $tareas = TareaSeguimiento::with(['empresa'])
            ...
            ->whereBetween('fecha_produccion', [$inicioSemana, $finSemana])
            //->where('estado_produccion', 'pendiente') // puedes ajustar este filtro según tus estados
            ->orderBy('fecha_produccion')
            ->get()
            ->groupBy(function ($tarea) {
                return Carbon::parse($tarea->fecha_produccion)->format('Y-m-d');
            });

        return inertia('Camarografo/TareasSemana', [
            'tareas' => $tareas,
        ]);
    } */
    public function tareasSemanaActual()
    {
        ...
        foreach ($tareas as $tarea) {
            $key = $tarea->empresa->id . '_' . Carbon::parse($tarea->fecha_produccion)->format('Y-m-d');
    ...
    public function tareasHoy()
    {
        ...
        foreach ($tareas as $tarea) {
            $key = $tarea->empresa->id . '_' . $tarea->fecha_produccion;
            $tarea->influencers = $bookings[$key] ?? collect();
        }

        return Inertia::render('Camarografo/TareasHoy', [
            'tareas' => $tareas,
            'hoy' => $hoy,
        ]);
    }
