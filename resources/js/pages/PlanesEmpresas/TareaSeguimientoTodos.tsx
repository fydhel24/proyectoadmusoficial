...
export default function SeguimientoTareasTodos({
    ...
    mes: string;
    anio: string;
    filtros?: { empresa?: string; titulo?: string }; // 👈 tipado
}) {
    const [empresaFiltro, setEmpresaFiltro] = useState(filtros?.empresa ?? '');
    const [tituloFiltro, setTituloFiltro] = useState(filtros?.titulo ?? '');

    const [nuevoTitulo, setNuevoTitulo] = useState('');
    const [nuevaSemana, setNuevaSemana] = useState('1');
    const columnaProduccion = 'bg-blue-50'; // azul clarito
    const columnaEdicion = 'bg-yellow-50'; // amarillo clarito
    const columnaEntrega = 'bg-green-50'; // verde clarito

    const handleBuscar = () => {
        router.get(route('seguimiento-tareas-todos'), {
            mes: mesFiltro,
            anio: anioFiltro,
        ...
        <AppLayout>
            ...
            <div className="p-6">
                <div className="mb-4 flex items-center justify-between border-b pb-4">
                    <h1 className="text-3xl font-bold text-gray-800">Registros del mes</h1>
                </div>
                ...
                <div className="mb-6 flex items-center space-x-4">
                    ...
                    <input
                        ...
                        className="rounded border px-2 py-1"
                    />
                    <button onClick={handleBuscar} className="rounded bg-green-500 px-4 py-2 text-white hover:bg-green-600">
                        Buscar
                    </button>
                </div>
