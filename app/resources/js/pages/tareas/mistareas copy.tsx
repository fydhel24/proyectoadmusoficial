...
export default function MisTareas() {
    ...
    // ✅ Inertia form hook - this will handle the form submission
    const {
        data: form,
        setData,
        post,
    ...
    // Filtrado de tareas - solo mostrar tareas del usuario actual
    const filteredTareas = useMemo(() => {
        const today = new Date().toISOString().split('T')[0];

        // Primero filtrar por fecha de hoy y por usuario actual
        let result = tareas.filter((t) => {
            const isToday = t.fecha === today;
            const isAssignedToCurrentUser = t.asignados.some((a) => a.user_id === currentUserId);
            return isToday && isAssignedToCurrentUser;
        });
        ...
        // Remover el filtro por usuario ya que solo mostramos tareas del usuario actual
        // if (filterUser) {
        //   result = result.filter((t) => t.asignados.some((a) => a.user_name === filterUser))
        // }

        return result;
    }, [tareas, searchTerm, filterPriority, currentUserId]);

    // Agrupar tareas por usuario - solo usuario actual
    const tareasPorUsuario = useMemo(() => {
        const mapa: Record<string, TareaAsignada[]> = {};

        filteredTareas.forEach((tarea) => {
            // Solo procesar asignaciones del usuario actual
            const asignacionUsuarioActual = tarea.asignados.find((asig) => asig.user_id === currentUserId);

            if (asignacionUsuarioActual) {
                const nombre = asignacionUsuarioActual.user_name;
                if (!mapa[nombre]) {
                    mapa[nombre] = [];
                }
    ...
    // ✅ Fixed functions for new task
    const startNewTask = () => {
        setShowNewTaskForm(true);
        // Reset the Inertia form with default values
        reset();
        setData({
            titulo: '',
            prioridad: '',
    ...
    const saveNewTask = (e: React.FormEvent) => {
        ...
        // Submit to the correct endpoint that matches your Laravel route
        post('/tareas-personales', {
            onSuccess: () => {
                setShowNewTaskForm(false);
                reset();
                setAsignarEmpresa('no');
            ...
            onError: (errors) => {
                ...
                setError('Error al crear la tarea. Verifica los datos ingresados.');
            },
        });
    };
    ...
    const handleChangeUser = async (taskId: number) => {
        ...
        try {
            ...
            const asignacionId = tarea.asignados[0].id;
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Tareas', href: '/tareas' }]}>
                    ...
                    <Fade in timeout={800}>
                        <Box>
                            ...
                            <Typography variant="subtitle1" sx={{ opacity: 0.9 }}>
                                Gestiona y edita tus tareas personales del día
                            </Typography>
            ...
            <Container maxWidth="xl" sx={{ py: 3 }}>
                                    ...
                                    <Grid container spacing={3}>
                                        ...
                                        <Grid item xs={12} md={6}>
                                            <Autocomplete
                                                ...
                                                value={tipos.find((t) => t.id === Number(form.tipo_id)) || null}
                                                onChange={(_, newValue) => setData('tipo_id', newValue ? String(newValue.id) : '')}
                                                sx={{ minWidth: 280 }}
                                                renderInput={(params) => (
                                                    <TextField
                                                        {...params}
                                        ...
                                        <Grid item xs={12} md={6}>
                                            <FormControl component="fieldset">
                                                ...
                                                <RadioGroup row value={form.prioridad} onChange={(e) => setData('prioridad', e.target.value)}>
                                        ...
                                        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                                            ...
                                            <Button
                                                ...
                                                variant="contained"
                                                startIcon={<Save />}
                                                disabled={processing || !form.titulo.trim()}
                                                sx={{
                                                    borderRadius: 2,
                                                    px: 3,
                                                    py: 1,
                ...
                {/* Rest of your existing code for editing, task display, etc. */}
                {/* ... (keeping the rest of the component as is) ... */}
